...
- **SynScan** - Sky-Watcher EQM-35 Pro via SynScan V4 hand controller (serial), telescope driver test results; site coordinates rounded to whole degrees
...
- **Scrub observing-site coordinates before committing a telescope report.** ConformU reads
  ...
  "Test value" lines). Round them to the nearest degree, keeping the hemisphere and rough
  region so the log stays coherent. This is a privacy matter for the contributor who ran the
  test, not a correctness one — the assertions the report exists to carry are unaffected.
