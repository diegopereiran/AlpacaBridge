...
For device behavior the YAML does not capture (capability gates, units, completion properties, error selection, ImageBytes, discovery), use the `ascom-alpaca-driver` skill in `.claude/skills/ascom-alpaca-driver/`, starting with its `references/alpacabridge-integration.md`.
