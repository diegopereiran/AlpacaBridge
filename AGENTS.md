...
- 16 hardware-free Catch2 cases: 11 driver-contract/frame-parsing/handshake cases (incl. the real captured frame above) plus 5 over a pty-backed fake hub (`tests/fake_gemini_pdh.h`: replies to `>H#`/`>V#`/`>G#`, never acks set commands, optionally streams `*G` frames) covering connect + first frame, every write path incl. the mode-dependent DEW range, streamed frames routed past the handshake, the 2 s stale re-poll, and the firmware gate. Plus routing/config round-trip.
