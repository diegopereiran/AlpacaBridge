...
- **No moving flag**: the protocol has none; INDI/INDIGO compare position to target. The driver reports `IsMoving` = (a target is pending AND position != target AND the position changed within the last 1.5 s), so a stalled or clamped move reads as stopped after one grace period instead of forever. `halt()` drops the target. A move to the current position answers `{"idx":6,"step":0,...}` and never moves.
...
- **The firmware is one reply behind on real USB (the tcdrain+newline-kick rule).** The GD32 CDC-ACM firmware transmits a reply only on a USB OUT-packet boundary and computes a command's reply *after* transmitting on that command's own OUT, so the reply to command N does not reach the host until the NEXT OUT is written. A naive write/read is therefore always exactly one reply behind. It looked fine on the dev VM only because VMware's USB proxy coalesced the traffic; a Raspberry Pi's native xHCI shows the real behaviour and the driver hangs at the second connect command (`{"cmd_id":7}` gets the handshake's `{"idx":1}`). `transact_locked()` fixes it: write the command, **`tcdrain()` it out as its own packet**, then write a newline "kick" (the parser waits for `{` and ignores the newline, but its OUT packet clocks out the pending reply), then read until the awaited `idx` appears, skipping the previous command's reply the kick shakes loose first. Do NOT try to "drain stale replies" in a loop: a newline never computes a new reply, so a bare kick loop re-reads the SAME reply forever (it wedged the connect once). Getting the drain/kick sequence wrong is invisible on the pty fake (it answers instantly, no OUT-boundary behaviour) and on the VM — only the Pi reproduces it. Also: the empty-slice fallback kick MUST come fast; an early version waited a 150 ms slice before kicking, which passed ConformU's error/issue checks but blew the 0.1 s FAST timing target on `DeviceState`/`IsMoving` (each read cost ~150 ms). tcdrain+proactive-kick brings each round trip to ~1 ms.
- **It is an extremely quiet, open-loop EAF.** No physical sound or vibration is perceptible during a move, and the reported position is an open-loop step counter (no encoder), so it advances whether or not the shaft turns. When validating, confirm motion by holding the gear, not by ear -- "not moving" was a false alarm on 2026-09-13 caused purely by how silent the unit is.
- **Hold settings are 12 V-only**: INDI gates `cmd_id` 12/16 on supply voltage > 11.5 V (`c_r`). The driver applies `reverse` and `speed` at every connect, and `holdForce`/`holdIhold`/`holdIrun` only when `c_r/10 > 11.5`, logging a WARN otherwise. All of these are config fields pushed at connect, not persisted device state.
- **Position is signed**: the firmware allows negative positions (INDI uses +/-64000). `move()` clamps to `[0, maxStep]` (config `maxStep`, default 64000) per ASCOM; a hardware position below zero is reported as-is.
- **Temperature source**: `temperatureSource` config picks `o_t` (external probe, default) or `c_t` (controller board). With no probe plugged in `o_t` still reports a plausible value, so the default is a judgment call, not a detection.
