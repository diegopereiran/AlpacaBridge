...
- **Connection**: USB/Serial or Wi-Fi (TCP). Auto-detection supported for both — `connectionType: "auto"` scans serial ports first, then falls back to network discovery if no serial mount is found. Each candidate port is probed at 9600 and then 115200 (an EQ board's built-in USB port answers only at 115200), so a Prolific/FTDI/CH340-class port with nothing Sky-Watcher on it costs about 3.3 s per scan; a rig with several such adapters takes a few seconds to auto-detect.
