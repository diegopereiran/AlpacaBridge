...
- **Site required**: The motor controller stores no site or time. Set Site Latitude/Longitude in the device config (or via the Alpaca setters) — with the site unset the driver refuses to connect (`InvalidOperation`), so nothing can run on a guessed 0/0 site.
